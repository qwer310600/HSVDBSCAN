...
def run_csp_solver(
    ...
    if solver_type == "naive":
        ...
        CSP_SOLVER.solve(query=query, select_solution=select_solution, correct_solution=correct_solution)
    else:
...
@register_handler()
class ConstraintColor(CSPConstraint):
    FUNC_NAME = "CONSTRAINT_COLOR"
    SIG_STR = "(target: CSPVar, condition: str) -> CSPConstraint"

    def call_type_check(self, target: CSPVar, condition: str) -> CSPConstraint:
        check(
            isinstance(target, CSPVar),
            f"parameter @target has wrong type: {type(target)}.",
        )
        check(
            ...
            f"parameter @condition has wrong type: {type(condition)}.",
        )
    ...
    def call(self, target: CSPVar, condition: str) -> CSPConstraint:
        con = self.__class__(target=target, condition=condition)
        CSP_SOLVER.add_constraint(constraint=con)
        return con

    def get_desc_str(self) -> str:
        return f"[target: {get_var_identifier(self.target)}] [condition: {self.condition}]"

    def __init__(self, target: CSPVar | None = None, condition: str | None = None):
        ...
        self.score_func = SCORE_FUNCTIONS["color"]
    ...
    def check_solution(self, solution_dict: Solution) -> bool:
        check(self.target in solution_dict)
        inst = solution_dict[self.target]
        
        scores = self.score_func.get_scores([inst], self.condition)
        #print("inst: ", inst.label, " ", inst.inst_id)
        #print("scores ", scores)
