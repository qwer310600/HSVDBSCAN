...
class CSPSolver:
    ...
    def solve(
        ...
        verbose: bool = False,
        correct_solution: Solution = None,
    ):
        assert query
        ...
        for sol_dict in solution_generator(csp_vars=normal_vars):
            ...
            for con in self.normal_constraints:
                if con.apparent_name == "CONSTRAINT_COLOR":
                    continue  
        ...
        if valid_solutions:
            ...
            if len(valid_solutions) > 1:
                if verbose:
                    print()
                    print("Applying color constraints to multiple valid solutions:")
                    print()


                filtered_solutions = []
                for sol_dict in valid_solutions:
                    con_failed = False
                    for con in self.normal_constraints:
                        if con.apparent_name != "CONSTRAINT_COLOR":
                            continue  # Apply color constraints now
                        
                        if not con.check_solution(sol_dict):
                            #if sol_dict[target_var].inst_id 
                                #print("\nasdfasdf", con.get_desc_str())
                            con_failed = True
                            break

                    if not con_failed:

                        filtered_solutions.append(sol_dict)


                if (len(filtered_solutions) != 0):
                    if len(filtered_solutions) < len(valid_solutions):
                        removed_solutions = [sol for sol in valid_solutions if sol not in filtered_solutions]
                        #print(removed_solutions)
                        # Check if the correct solution is in valid_solutions but not in filtered_solutions
                        
                        #if any(correct_solution["target_id"] == sol[target_var].inst_id for sol in removed_solutions):
                        #    print("\nwarning1: The correct solution was removed!")
                        
                        #print("\nwarning2: color used")
                        valid_solutions = filtered_solutions

                if verbose:
                    print()
                    print("valid solutions after applying color constraints:")
                    for sol in valid_solutions:
                        print(solution_to_str(sol))
